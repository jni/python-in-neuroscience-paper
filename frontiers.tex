...
For example, after determining suitable watershed parameters in an IPython environment (REF), these two function calls gave our top-ranked entry to the SNEMI3D challenge (REF):
...
As a workaround for gala's memory efficiency issues, we developed an alternative interface called NeuroProof \citep{np}, which can be used as the backend for the gala algorithm by calling the \texttt{Stack} class in \texttt{gala.stack\_np} instead of \texttt{agglo.Rag}.
