...
This has enabled less obvious features such as many based on the convex hull of the segment.
The convex hull feature manager stores as a cache the convex hull of each node.
When two nodes are merged, the resulting convex hull can be computed faster by starting from the two initial hulls, rather than the newly formed segment.
...
However, we later realised that this is only the endpoint of a continuous set of features that we called "inclusiveness".
